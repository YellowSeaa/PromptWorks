...
## 9. MVP 推进建议
1. 确认模块协议与 `AnalysisContext` 设计，编写开发者文档和脚手架示例。
2. 实现后端模块注册/执行框架（内置正则命中、成功率分布等 2-3 个示例模块）。
3. 前端完成分析卡片组件、参数表单渲染、图表映射逻辑。
4. 打通 LLM 调用流程，并在示例模块中演示摘要能力。
5. 建立执行日志与错误反馈渠道，确保用户调试体验。
