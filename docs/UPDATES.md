# 更新记录

- 当前版本: 0.7.1

## 0.7.1 (发布日期: 2026-06-09)
- 测试任务结果页合并 AI 评分与优化建议入口，保留评分/优化两个核心操作，减少结果页信息噪声。
- 新增 AI 优化工作台页面，支持基于评分摘要查看问题归因、生成 Prompt 改写、人工编辑并创建新版本。
- Prompt 改写结果支持一键新增为 Prompt 管理中的新版本，创建后自动激活并跳转 Prompt 详情。
- 明确 Prompt 测试任务只归属于一个 Prompt，后端会拒绝跨 Prompt 版本混入同一测试任务或追加单元。
- 顶部导航版本号更新为 v0.7.1。
- README 与英文版 README_en 补充 AI 评分与优化工作台作为核心能力亮点，并加入中英文页面截图展示。
- 修复前端 Docker 多架构构建在 arm64 QEMU 环境中执行 Vite 构建导致 Illegal instruction 的问题。

## 0.7.0 (发布日期: 2026-06-09)
- Prompt 测试任务新增 AI 评分能力，可在创建任务时开启，也可在结果页对既有任务补跑或重跑评分。
- 测试输出产出后会立即触发评分，评分状态独立于测试任务状态展示，单条评分失败不会影响测试任务完成。
- AI 评分与测试执行按模型并发上限并行调度，同模型场景会拆分共享并发，单并发模型保持串行共享。
- 修复异步 AI 评分未预创建待评分记录导致结果页评分数量少于测试输出、进度误显示为已完成的问题。
- 修复测试任务完成但 AI 评分仍有待处理记录时，结果页按钮误显示“开始评分”的问题。
- AI 评分提示词统一要求 0-100 百分制，并在入库前校验疑似 5 分制或 10 分制响应，异常评分会触发重试或失败。
- 测试结果页优化 AI 评分卡片布局，仅保留一个状态化操作按钮，并在任务运行评分中禁止手动重复触发。
- 测试结果页的最小测试单元卡片新增平均分展示，单元详情页新增每条测试结果的 AI 评分展示。
- 修复单元详情页变量筛选下拉框宽度过窄的问题。
- 重新评分会持续写入可见进度，刷新页面后仍显示评分中并禁用重复触发按钮。
- 顶部导航新增当前版本号展示，便于确认正在使用的 PromptWorks 版本。
- 结果页新增评分徽标、悬浮评分明细、失败重试、评分进度、单元统计和手动优化推荐入口。
- AI 优化推荐会基于评分明细、单元统计、Prompt、模型与参数生成参数建议、模型建议和 Prompt 迭代草稿。
- 修复 AI 优化推荐在模型读超时时直接展示底层英文错误的问题，并压缩推荐请求中的测试输出样例以降低超时概率。
- 运行超时设置新增 AI 优化建议超时时间，避免长上下文建议生成与测试任务超时配置相互影响。
- 前端构建新增依赖分包策略，拆分 Vue、Element Plus、ECharts 等公共依赖，降低单个 chunk 体积。
- 补充 LLM 上下文截断与分析模块注册表单元测试，提升关键服务逻辑覆盖率。

## 0.6.5 (发布日期: 2026-06-09)
- 修复 Prompt 测试任务删除后后端仍继续执行的问题，任务被软删除后会取消未完成实验并停止后续执行。
- Prompt 测试实验支持断点续跑，已完成轮次会保留并跳过，失败或中断后可继续补跑剩余轮次。
- Prompt 测试任务运行中会逐轮持久化结果，详情页可查看已完成输出。
- LLM 模型配置新增上下文长度字段，快速测试、旧测试任务与 Prompt 测试任务会按模型配置进行近似上下文截断。

## 0.6.4 (发布日期: 2026-06-09)
- 修复 Prompt 测试任务未应用 LLM 模型并发配置的问题，执行轮次现会按模型 `concurrency_limit` 并发调用。
- 新增 Prompt 测试并发限制回归测试，确保配置为 2 时最多同时执行 2 个 LLM 请求。

## 0.6.3 (发布日期: 2025-11-08)
- CI 现会分别构建并推送后端与前端镜像到同一个仓库 `yellowseaa/promptworks`，标签以 `backend-*` / `frontend-*` + 渠道后缀区分。
- `docker-compose.yml` 默认引用远端镜像并支持通过 `BACKEND_IMAGE`、`FRONTEND_IMAGE` 环境变量切换 main/dev 或特定版本。
- README 与 README_en 的 Docker 指南同步更新，强调 `docker compose pull + up -d` 即可获取完整栈镜像。

## 0.6.2 (发布日期: 2025-11-08)
- README 与英文版 README_en 将 Docker 部署移动到快速开始首位，并推荐直接拉取 `yellowseaa/promptworks` 镜像实现一键体验。
- 更新快速开始说明，区分推荐镜像方式与本地源码方式，帮助团队统一部署路径。

## 0.6.1 (发布日期: 2025-11-08)
- 「耗时与 Tokens 概览」分析模块仅统计成功样本，失败轮次不再写入耗时 / Tokens 聚合，确保成本与性能读数准确。
- 洞察面板将平均 tokens 指标调整为强调最低耗用单元，便于快速定位成本最佳的 Prompt 方案。
- 测试结果页在「最小测试单元 / 测试结果 / 分析报告」切换区域新增「重试任务」按钮，可一键跳转到新建测试任务并继承原任务配置。
- 新建测试任务页识别 `retryTaskId` 查询参数后，会自动填充任务名称（追加 `-retry`）、Prompt 版本、模型、参数集以及变量样本，确保重试任务配置秒级复用。

## 0.6.0 (发布日期: 2025-11-08)
- 新增「分析报告」功能，支持将多模块结果合并为交互式报告。
- 报告模板支持自定义模块顺序、附加解读与指标卡片。

## 0.5.4 (发布日期: 2025-11-07)
- Prompt 详情页的测试记录新增「任务名称」列，方便在同一视图快速定位任务来源并跳转到结果页。
- 补充 Prompt 测试任务 API 的多项集成测试，覆盖状态筛选、任务与单元 CRUD、异常重跑等场景，总体覆盖率提升至 92%。

## 0.5.3 (发布日期: 2025-11-07)
- Prompt 测试任务中的请求即使个别失败也会继续执行，并在任务列表与结果页展示对应的失败原因，方便定位网络或模型异常。

## 0.5.2 (发布日期: 2025-11-01)
- 快速测试功能流式输出新增首字响应时间和总耗时显示，非流式输出新增总耗时显示
- 快速测试功能新建对话会保留原来的模型与参数

## 0.5.1 (发布日期: 2025-11-01)
- 修复测试任务进度条总数统计错误，现按「最小测试单元 × 执行轮次 × 测试样本量」精准计算，并同步前后端逻辑。
- Prompt 测试结果页与单元详情页在输出为空时新增原因提示，覆盖超时、执行失败、排队等场景，便于快速定位问题。

## 0.5.0 (发布日期: 2025-11-01)
- 快速测试与测试任务支持自定义超时时间，全局系统设置引入持久化存储与 API，统一前后端行为。
- 顶部新增设置弹窗，可配置超时并查看更新时间，流式与非流式请求均同步使用最新值。
- 测试任务与 Prompt 测试流程读取系统配置实时调整调用超时，新增单元测试覆盖配置场景。

## 0.4.5 (发布日期: 2025-10-30)
- Prompt 测试队列新增进度追踪器，每完成 5% 轮次即写入任务配置，确保前端可实时呈现进度并在完成/失败时强制写满 100%。
- Prompt 测试实验执行流程支持进度回调，逐轮上报完成情况，前端任务列表进度条即时更新。
- 修复测试任务操作列 `el-space` 尺寸告警，任务列表在执行过程中展示「已完成/总数」与进度条。
- Prompt 测试结果页与最小测试单元详情页新增 Markdown 渲染开关，支持按需以富文本方式查看模型输出。
- 当 LLM 响应返回 200 但解析失败时，前端展示告警卡片并可查看原始响应详情，便于排查异常格式。

## 0.4.4 (发布日期: 2025-10-30)
- 快速测试对话流式渲染支持实时逐字输出，启用气泡打字机与加载动效，优化等待体验。
- 流式接口增加 `keep-alive` 响应头并完善调试日志，前端新增 SSE/字符级调试输出，便于追踪链路。

## 0.4.3 (发布日期: 2025-10-27)
- 统一 Git 行尾策略为 LF，新建 `.gitattributes` 配置并重归一化仓库文件，避免跨平台换行差异。
- 修复 Prompt 详情页无法跳转测试记录的问题，兼容新旧测试任务入口。
- 调整用量管理页布局，宽屏下模型与趋势卡片各占半屏保持对齐。
- 将测试运行和提示实验中快照注入逻辑统一改为 user 角色，避免默认写入 system 导致模型忽略消息

## 0.4.2 (发布日期: 2025-10-26)
- 修复 `BACKEND_CORS_ORIGINS` 环境变量解析，兼容字符串写法，避免首次迁移失败
- 更新中英文 README 快速开始指引，补充 PostgreSQL 数据库初始化步骤

## 0.4.1 (发布日期: 2025-10-25)
- 调整 Prompt 测试任务的删除逻辑，新增 `is_deleted` 字段，前端删除默认软删除，历史数据仍然保留
- Prompt 测试执行流程自动写入 LLM 使用日志，用量管理面板能够统计测试任务的消耗
- 优化测试任务执行队列，当任务已被软删除时自动跳过，避免重复触发

## 0.4.0 (发布日期: 2025-10-24)
- 增加测试任务删除接口与前端操作入口，支持快速清理冗余任务。
- 最小测试单元结果页新增变量查看和筛选能力，便于按样本定位输出。
- 修复逻辑：测试任务中，若有测试样本（变量），则测试次数为变量行数*执行轮次。

## 0.3.1 (发布日期: 2025-10-24)
- 修复新建测试任务时单元名前缀仅同步首字符的问题，并将 Poe 任务改为跨平台命令。

## 0.3.0 (发布日期: 2025-10-24)
- Prompt 详情页新增复制功能 (a28bea4)。
- 同步全局 AGENTS 协议规范，统一团队协作语义 (f4bf657)。

## 0.2.0 (发布日期: 2025-10-23)
- 扩展模型提供方配置，补齐官方指标字段，完善运营看板 (7400f80)。
- 优化 Prompt 分类与筛选体验，支持批量管理与导出 (dd6c79b)。

## 0.1.0-20251012 (发布日期: 2025-10-12)
- 新增批量导入脚本，支持快速初始化测试数据 (242687c)。
- 调整平台 Logo、README 及使用指南，完善产品对外形象 (93580ec, 0e742f7)。
- 补全 LICENSE，明确开源协议 (24b8490)。

## 0.1.0-20251011 (发布日期: 2025-10-11)
- 打通 Prompt 管理前端、后端与历史记录，搭建完整工作流 (40f7e76, 0cc85dd, 4ee48d7)。
- 支持多模型多版本稳定对比流程，提升串行测试能力 (583f17e, 2aee9f0)。
- 优化调度与错误处理，增强任务执行鲁棒性 (33f456a)。

## 0.1.0-20251005 (发布日期: 2025-10-05)
- 将 Prompt 测试任务改为异步执行并提供日志追踪 (c0f2078, 812ac01)。
- 修复多处空指针问题并完善异常提示 (aed169d)。
- 丰富国际化翻译内容，补齐英文文案 (59ce32b)。
- 调整 Poe 工具链配置，统一依赖版本 (0fa16d2)。

## 0.1.0-20251004 (发布日期: 2025-10-04)
- 打磨快速测试、历史记录等核心能力，提升研发效率 (62d8bc4, 4ee48d7, 3a0e743)。
- 补充中英文文档与 API 指引，完善用户手册 (9ea9af3, 8e3dd49, d21a0d1)。
- 提供多模型多版本的对比视图 (2aee9f0)。
- 优化运营报表与告警流程 (b410bd0, 3fb91ce)。

## 0.1.0-20251003 (发布日期: 2025-10-03)
- 完善 LLM 配置、标签管理等基础页面 (910a177, 7404b31, 232f97b)。
- 丰富 Prompt 管理页交互，提供更多扩展入口 (117f451, 8c1e3bf)。
- 改善模型列表分页与路由体验 (af1c601, 9845b54)。

## 0.1.0-20250929 (发布日期: 2025-09-29)
- 打通后端 Prompt 管理接口，补齐数据闭环 (aaed330)。

## 0.1.0-20250926 (发布日期: 2025-09-26)
- 上线 LLM 管理页面文件导入与导出能力 (c6f12f4)。
- 统一代码格式化与校验规范 (083b4fe)。

## 0.1.0-20250925 (发布日期: 2025-09-25)
- 迭代快速测试、参数模板与任务队列模块，为性能评估打底 (b40b8cd)。

## 0.1.0-20250924 (发布日期: 2025-09-24)
- 初始化前端项目骨架及 Prompt 展示页面 (4f64b3f, f870b95, de14e4a)。
- 完成 Prompt 服务基础 CRUD 能力 (72c1b36)。

## 0.1.0-20250920 (发布日期: 2025-09-20)
- 设计 Prompt 数据模型为「分类-提示-版本」结构，增强灵活性 (ce6c4a9)。
- 构建 Prompt 标签接口与业务逻辑 (ca3620b)。
- 实现 LLM API 初版与联调文档 (d028e09)。

## 0.1.0-20250919 (发布日期: 2025-09-19)
- 发布 `test_prompt` 系列 API，打通测试任务流程 (b1120b7)。

## 0.1.0-20250918 (发布日期: 2025-09-18)
- 搭建运营看板初版，汇总 LLM 使用指标 (f2779ec)。
- 引入格式化与静态检查工具，统一编码规范 (a9ca789, 50a2f62)。
- 调整告警处理与日志注释，提升可维护性 (fb0bbdf)。

## 0.1.0-20250917 (发布日期: 2025-09-17)
- 集成 Alembic 并初始化数据库模型 (8e1c059)。
- 构建 LLM 提供方管理与日志体系 (9f83a60, dedfe73, 1f66d7c)。
- 初始化仓库并提交首个工作流配置 (593b481)。
