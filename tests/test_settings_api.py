...
def test_get_testing_timeouts_returns_defaults(client):
    ...
    assert data["ai_optimization_timeout"] == 180
...
def test_update_testing_timeouts(client, db_session):
    response = client.put(
        ...
        json={
            "quick_test_timeout": 40,
            "test_task_timeout": 150,
    ...
    assert data["ai_optimization_timeout"] == 240
    ...
    assert record.value == {
        ...
        "ai_optimization_timeout": 240.0,
    }
    assert isinstance(record.updated_at, datetime)


def test_update_testing_timeouts_accepts_legacy_payload(client):
    response = client.put(
        "/api/v1/settings/testing",
